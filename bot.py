...
def cmd_check(update: Update, context: CallbackContext):
    ...
    kwork_check_job(context)  # синхронный вызов для быстрого ответа
...
# ---------- ПАРСИНГ KWORK С CSRF-ТОКЕНОМ ----------
KWORK_SESSION = None

def get_kwork_session() -> requests.Session:
    global KWORK_SESSION
    if KWORK_SESSION is None:
        KWORK_SESSION = requests.Session()
        KWORK_SESSION.headers.update({
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        })
    return KWORK_SESSION

def login_kwork() -> bool:
    session = get_kwork_session()
    login = KWORK_LOGIN
    password = get_kwork_password()
    if not login or not password:
        logger.warning("Логин или пароль Kwork не заданы.")
        return False

    try:
        # 1. GET-запрос к странице входа для получения CSRF-токена
        resp = session.get("https://kwork.ru/login", timeout=10)
        ...
        if not csrf_token:
            # Пробуем другие возможные имена поля
            token_input = soup.find("input", {"name": "_token"})
            if token_input:
                csrf_token = token_input.get("value")
        if not csrf_token:
            logger.error("Не удалось найти CSRF-токен на странице входа")
            return False

        # 2. POST-запрос с логином, паролем и токеном
        payload = {
            "login": login,
            "password": password,
        ...
        login_resp = session.post("https://kwork.ru/login", data=payload, timeout=10)
        login_resp.raise_for_status()
        # Проверяем успешность: редирект на главную или наличие logout
        if login_resp.url == "https://kwork.ru/" or "logout" in login_resp.text.lower():
            logger.info("✅ Авторизация Kwork успешна (CSRF-токен)")
            return True
        else:
...
def search_kwork(keyword: str) -> list:
    session = get_kwork_session()
    try:
        resp = session.get("https://kwork.ru/projects", params={"a": 1, "keyword": keyword}, timeout=15)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")
        results = []
        # Ищем карточки проектов – используем несколько популярных селекторов
        cards = soup.select("div.want-card, div[class*='project-card'], article, div.project-item")
        for card in cards:
        ...
        logger.error(f"Ошибка поиска Kwork по '{keyword}': {e}")
        return []

# Глобальный флаг авторизации
kwork_authorized = False

def ensure_kwork_auth():
    global kwork_authorized
    if not kwork_authorized:
        kwork_authorized = login_kwork()
    return kwork_authorized

def kwork_check_job(context: CallbackContext):
    if data["paused"] or not in_work_hours():
        return

    if not ensure_kwork_auth():
...
def main():
    ...
    # Попробуем авторизоваться при старте (необязательно, но полезно)
    ensure_kwork_auth()
