...
def cmd_check(update: Update, context: CallbackContext):
    ...
    # Вызываем задачу синхронно (без потока) чтобы видеть ошибки
...
def search_kwork(keyword: str, session: requests.Session) -> list:
    ...
    try:
        ...
        # Ищем карточки проектов (селекторы могут меняться, но базовые)
...
def kwork_check_job(context: CallbackContext):
    ...
    # Авторизуемся (если логин/пароль заданы)
    login_kwork(session)
...
def main():
    ...
    # Запускаем планировщик с интервалом KWORK_CHECK_INTERVAL
